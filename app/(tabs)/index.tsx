...
export default function DashboardScreen() {
  ...
  return (
    <View className="flex-1 bg-[#F5F7FA]">
      <View className="px-6 pt-12 pb-6 bg-white rounded-b-[30px] shadow-sm">
      ...
      <ScrollView className="flex-1 px-6 mt-6" showsVerticalScrollIndicator={false}>
        <View className="flex-row flex-wrap justify-between">
          {[
            ...
            <View key={i} className="bg-white p-5 rounded-3xl border border-gray-100 w-[47%] mb-4 shadow-sm">
        ...
        <View className="bg-white rounded-3xl p-2 border border-gray-100 shadow-sm mb-10">
          {filteredData.slice(0, 8).map((item) => (
            <View key={item.id} className="flex-row items-center p-4 border-b border-gray-50 last:border-0">
              <View className="w-10 h-10 bg-purple-50 rounded-xl justify-center items-center">
                <Text className="text-purple-600 font-bold text-[10px]">#{item.id.toString().slice(-2)}</Text>
              </View>
              ...
              <Text className="font-bold text-[#633594] text-xs">Rp {(parseInt(item.total_price) / 1000).toFixed(0)}K</Text>
            </View>
          ))}
        </View>
      </ScrollView>
